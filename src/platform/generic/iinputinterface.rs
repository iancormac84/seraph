...
impl HapticFeedbackValues {
    ...
    pub fn from_freq_and_amplitude(in_frequency: f32, in_amplitude: f32) -> HapticFeedbackValues {
		HapticFeedbackValues {
			frequency: if in_frequency < 0.0f32 { 0.0f32 } else if in_frequency > 1.0f32 { 1.0f32 } else { in_frequency },
		    amplitude: if in_amplitude < 0.0f32 { 0.0f32 } else if in_amplitude > 1.0f32 { 1.0f32 } else { in_amplitude },
		    haptic_buffer: ptr::null_mut(),
		}
	}
}
...
pub trait IInputIterface {
	fn set_force_feedback_channel_value(&mut self, controller_id: i32, channel_type: ForceFeedbackChannelType, value: f32);
	fn set_force_feedback_channel_values(&mut self, controller_id: i32, values: &ForceFeedbackValues);
	fn set_haptic_feedback_values(&mut self, controller_id: i32, hand: i32, values: &HapticFeedbackValues);
